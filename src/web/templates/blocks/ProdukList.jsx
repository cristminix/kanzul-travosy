                    ...
                    <div className={cls5} key={index}>
                      ...
                      <div className={cls10}>
                        <div className={cls11}>
                          ...
                          <span className={`${cls14} twx-flex twx-items-center`}>
                            <IconUser className="feather-icon inline-block twx-text-black !twx-w-[14.6px] !twx-h-[14.6px]" />
                            <a href={`/produk/#/kategori/${item.kategori}`} className={cls15}>
                              {item.kategori}
                            </a>
                          </span>
                        </div>
                        {
                          /* item.compiledHash */ false ? (
                            <a href={`/produk/lihat/${item.id}/${slug}`} className={cls16}>
                              {item.title}
                            </a>
                          ) : (
                            <Link to={`/lihat/${item.id}/${slug}`} className={cls16}>
                              {item.title}
                            </Link>
                          )
                        }
                        <p className={`${cls17} twx-line-clamp-3`}> {item.headline}</p>

                        <div className={cls18}>
                          {
                            /* item.compiledHash */ false ? (
                              <a href={`/produk/lihat/${item.id}/${slug}`} className={cls19}>
                                Selengkapnya
                                <svg
                                  xmlns="http://www.w3.org/2000/svg"
                                  width="24"
                              ...
                              <Link to={`/lihat/${item.id}/${slug}`} className={cls19}>
                                Selengkapnya
                                <svg
                                  xmlns="http://www.w3.org/2000/svg"
                                  width="24"

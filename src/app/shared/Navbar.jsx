...
const Navbar = ({}) => {
  ...
  const repairGitDataStore = () => {
    const taskId = crc32id()
    const logs = {
      [taskId]: "repair",
    }
    setDoctorTask({ lastId: taskId })
    setDoctorLogs((oData) => ({ ...oData, ...logs }))
  }
  ...
  return (
    <nav className="navbar default-layout-navbar col-lg-12 col-12 p-0 fixed-top d-flex flex-row">
      <div className="text-center navbar-brand-wrapper d-flex align-items-center">
        <button
          className="navbar-toggler align-self-center left-toggler"
          type="button"
          onClick={() => document.body.classList.toggle("sidebar-icon-only")}>
          <span className="mdi mdi-menu"></span>
        </button>
        <Link className="navbar-brand brand-logo mobile" to="/">
          <img className="" src={"/assets/images/logo/logo-dark.png"} alt="logo" />
        </Link>
        <Link className="navbar-brand brand-logo" to="/">
          <img className="first" src={"/assets/images/logo/logo-dark.png"} alt="logo" />
        </Link>
        <Link className="navbar-brand brand-logo-mini" to="/">
          <img className="second" src={"/assets/images/logo/logo-dark.png"} alt="logo" />
        </Link>
      </div>
      ...
      <GitDoctor task={doctorTask} logs={doctorLogs} />
    </nav>
  )
}
